...
if __name__ == "__main__":
    ...
    # Setting EvalCallback
    eval_callback = EvalCallback(
        eval_env=eval_env,
        eval_freq=config["RL"]["eval_freq"],  # Evaluate at the frequency specified in the config
        n_eval_episodes=config["RL"][
            "n_eval_episodes"
        ],  # Evaluate based on the number of episodes specified in the config
        log_path=os.path.join(os.path.dirname(__file__), "..", config["RL"]["eval_log_path"]),  # Log Save Location
        best_model_save_path=os.path.join(
            os.path.dirname(__file__), "..", config["RL"]["eval_log_path"], "best_model"
        ),  # Best model Save Location
        deterministic=True,  # Deterministic actions are used during evaluation.
        render=False,  # Rendering is off
    )

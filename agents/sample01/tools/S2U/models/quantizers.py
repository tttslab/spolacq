...
class VectorQuantizerEMA(nn.Module):
    def __init__(
        ...
        self.register_buffer("_ema_cluster_size", init_ema_mass * torch.ones(num_embeddings))
        self._ema_w = nn.Parameter(
            init_ema_mass * torch.from_numpy(self._embedding.weight.cpu().numpy()), requires_grad=False
        )
        if nonneg_init:
    ...
    def forward(self, inputs):
        ...
        onehots = torch.zeros(encoding_indices.shape[0], self._num_embeddings)
    ...
    def ema_update(self, inputs, onehots):
        ...
        self._ema_cluster_size = self._ema_cluster_size * self._decay + (1 - self._decay) * torch.sum(onehots, 0)
        ...
        self._ema_w = nn.Parameter(self._ema_w * self._decay + (1 - self._decay) * dw, requires_grad=False)

        self._embedding.weight = nn.Parameter(self._ema_w / self._ema_cluster_size.unsqueeze(1), requires_grad=False)

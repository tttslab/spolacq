...
def train(audio_model, image_model, train_loader, test_loader, args, exp_dir, resume):
    ...
    # Create/load exp
    if resume:
        (progress, epoch, global_step, best_epoch, best_acc) = load_progress("%s/progress.pkl" % exp_dir)
    ...
    if epoch != 0:
        audio_model.load_state_dict(torch.load("%s/models/audio_model.iter.pth" % (exp_dir)))
        image_model.load_state_dict(torch.load("%s/models/image_model.iter.pth" % (exp_dir)))
    ...
    if args.freeze_image_model:
        image_trainables = [p for n, p in image_model.named_parameters() if n.startswith("embedder")]
    else:
    ...
    if epoch != 0:
        optimizer.load_state_dict(torch.load("%s/models/optim_state.iter.pth" % (exp_dir)))
        ...
        torch.save(audio_model.state_dict(), "%s/models/audio_model.e%d.pth" % (exp_dir, epoch))
        torch.save(image_model.state_dict(), "%s/models/image_model.e%d.pth" % (exp_dir, epoch))
    epoch += 1
    ...
    while epoch <= args.n_epochs:
        ...
        for i, (image_input, audio_input, nframes) in enumerate(train_loader):
            ...
            (audio_output, quant_losses, flat_inputs, flat_onehots) = audio_model(audio_input, nframes)
            ...
            S = compute_pooldot_similarity_matrix(image_output, audio_output, nframes)
            ...
            loss = I2A_sampled_loss + A2I_sampled_loss + I2A_hardneg_loss + A2I_hardneg_loss
        ...
        print(
            "Finished epoch %d. Time elapsed = %.fs. Current Time = %s"
            % (epoch, time.time() - epoch_time, datetime.datetime.now())
        )

        torch.save(audio_model.state_dict(), "%s/models/audio_model.iter.pth" % (exp_dir))
        torch.save(image_model.state_dict(), "%s/models/image_model.iter.pth" % (exp_dir))
        torch.save(optimizer.state_dict(), "%s/models/optim_state.iter.pth" % (exp_dir))

        if avg_acc > best_acc:
            best_epoch = epoch
            best_acc = avg_acc
            shutil.copyfile("%s/models/audio_model.iter.pth" % (exp_dir), "%s/models/best_audio_model.pth" % (exp_dir))
            shutil.copyfile("%s/models/image_model.iter.pth" % (exp_dir), "%s/models/best_image_model.pth" % (exp_dir))

        if args.save_every > 0 and epoch % args.save_every == 0:
            shutil.copyfile(
                "%s/models/audio_model.iter.pth" % (exp_dir), "%s/models/audio_model.e%d.pth" % (exp_dir, epoch)
            )
            shutil.copyfile(
                "%s/models/image_model.iter.pth" % (exp_dir), "%s/models/image_model.e%d.pth" % (exp_dir, epoch)
            )

        _save_progress()
        epoch += 1

    print("Finished training. best_epoch = %s, best_acc = %s" % (best_epoch, best_acc))
...
def validate(audio_model, image_model, val_loader, args):
    ...
    print(
        " * Audio R@10 {A_r10:.3f} / Image R@10 {I_r10:.3f} / Mean R@10 {M_r10:.3f} over {N:d} validation pairs".format(
            A_r10=A_r10, I_r10=I_r10, M_r10=M_r10, N=N_examples
        ),
        flush=True,
    )
    print(
        " * Audio R@5 {A_r5:.3f} / Image R@5 {I_r5:.3f} / Mean R@5 {M_r5:.3f} over {N:d} validation pairs".format(
            A_r5=A_r5, I_r5=I_r5, M_r5=M_r5, N=N_examples
        ),
        flush=True,
    )
    print(
        " * Audio R@1 {A_r1:.3f} / Image R@1 {I_r1:.3f} / Mean R@1 {M_r1:.3f} over {N:d} validation pairs".format(
            A_r1=A_r1, I_r1=I_r1, M_r1=M_r1, N=N_examples
        ),
        flush=True,
    )

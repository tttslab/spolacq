...
def get_feats_codes(audio_model, layer, mels, device, mel_spf=0.01):
    ...
    with torch.no_grad():
        ...
        _, feats, pre_feats, onehots = audio_model.get_vq_outputs(mels, layer, unflatten=True)
...
def comp_code_to_wordprec(utt2codes, utt2words, code_halfwidth, stop_words):
    ...
    for utt_index in utt2codes:
        ...
        if word_ali is None:
            continue
        for code, seg_wordset in align_sparse_to_dense(word_ali, code_ali, center_to_range):
    ...
    for code, nsegs in sorted(code_to_nsegs.items()):
        ...
        word_prec_list = [
            (word, float(occ) / nsegs, occ) for word, occ in word_counts.most_common() if word not in stop_words
        ]
        n_codes_with_no_words += int(not bool(word_prec_list))
        code_to_wordprec[code] = word_prec_list

    print(
        "%d / %d codes mapped to only utterances with empty transcripts" % (n_codes_with_no_words, len(code_to_nsegs))
    )
...
def get_high_prec_words(code_to_wordprec, threshold=0.35):
    ...
    print("#" * 5, "Total %d words with precision >= %s" % (len(high_prec_words), threshold))
...
def comp_word_to_coderecall(utt2codes, utt2words, target_words):
    ...
    for word, nsegs in word_to_nsegs.items():
        ...
        code_recall_list = [(code, float(occ) / nsegs, occ) for code, occ in code_counts.most_common()]
...
def comp_code_word_f1(code_to_wordprec, word_to_coderecall, min_occ):
    ...
    for word in word_to_coderecall:
        ...
        word_to_code2recall[word] = {code: (recall, occ) for code, recall, occ in coderecall}
...
# Print Utils #
###############
def print_code_to_word_prec(code_to_wordprec, prec_threshold=0.35, num_show=3, show_all=True):
    ...
    print(
        ("Found %d / %d (%.2f%%) codes with a word detector withprec greater than %f.")
        % (n_codes_above_prec_threshold, n_codes, n_codes_above_prec_threshold / n_codes * 100, prec_threshold)
    )
...
def print_word_by_code_recall(word_to_coderecall, num_show=3):
    for word in sorted(word_to_coderecall):
        ...
        print(
            "%-15s (#occ = %4d)" % (word, tot_occ),
            [("%4d" % c, "%.2f" % r) for c, r, _ in word_to_coderecall[word][:num_show]],
        )


def print_code_stats_by_f1(code_to_wordf1, code_ranks_show=range(10), num_word_show=2):
    print("##### Showing ranks %s" % str(code_ranks_show))
    codes = sorted(code_to_wordf1.keys(), key=lambda x: (-code_to_wordf1[x][0][1] if len(code_to_wordf1[x]) else 0))
    print("%3s & %4s & %10s & %6s & %6s & %6s & %5s" % ("rk", "code", "word", "F1", "Prec", "Recall", "Occ"))
    for rank in code_ranks_show:
        ...
        for word, f1, prec, recall, occ in code_to_wordf1[code][:num_word_show]:
            msg += " & %10s & %6.2f & %6.2f & %6.2f & %5d" % (word.lower(), f1 * 100, prec * 100, recall * 100, occ)
        msg += " \\\\"
        print(msg)


def print_word_stats_by_f1(word_to_codef1, word_ranks_show=range(10), num_code_show=3):
    print("##### Showing ranks %s" % str(word_ranks_show))
    words = sorted(word_to_codef1.keys(), key=lambda x: (-word_to_codef1[x][0][1] if len(word_to_codef1[x]) else 0))
    print("%3s & %15s & %4s & %6s & %6s & %6s & %5s" % ("rk", "word", "code", "F1", "Prec", "Recall", "Occ"))
    for rank in word_ranks_show:
        ...
        for code, f1, prec, recall, occ in word_to_codef1[word][:num_code_show]:
            msg += " & %4d & %6.2f & %6.2f & %6.2f & %5d" % (code, f1 * 100, prec * 100, recall * 100, occ)
        msg += " \\\\"
        print(msg)
...
def count_high_f1_words(word_to_codef1, f1_threshold=0.5, verbose=True):
    ...
    for word in word_to_codef1.keys():
        if len(word_to_codef1[word]) and (word_to_codef1[word][0][1] >= f1_threshold):
            count += 1
    if verbose:
        print("%d / %d words with an F1 score >= %s" % (count, len(word_to_codef1), f1_threshold))
    return count


def compute_topk_avg_f1(word_to_codef1, k=250, verbose=True):
    f1s = [word_to_codef1[word][0][1] for word in word_to_codef1 if len(word_to_codef1[word])]
    top_f1s = sorted(f1s, reverse=True)[:k]

    if verbose:
        print(
            "avg F1 = %.2f%% for top %d words; %.2f%% for all %d words"
            % (100 * np.mean(top_f1s), len(top_f1s), 100 * np.mean(f1s), len(f1s))
        )
    return 100 * np.mean(top_f1s)


def print_analysis(code_to_wordprec, word_to_coderecall, code_norms, high_prec_words, min_occ, rank_range):
    print_code_to_word_prec(code_to_wordprec, prec_threshold=0.35, num_show=3, show_all=True)
    print_word_by_code_recall(word_to_coderecall, num_show=3)

    code_to_wordf1, word_to_codef1 = comp_code_word_f1(code_to_wordprec, word_to_coderecall, min_occ=min_occ)

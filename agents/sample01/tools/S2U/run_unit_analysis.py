...
import dataloaders
import models
from steps.unit_analysis import (
...
def prepare_data(audio_model, layer, dataset, json_data, max_n_utts):
    ...
    for utt_index in range(n_utts):
        utt2codes[utt_index] = get_code_ali(audio_model, layer, dataset, utt_index, device)
        utt2words[utt_index] = get_word_ali(json_data, utt_index)
        tot_nframes += len(utt2codes[utt_index])
        if (utt_index + 1) % 1000 == 0:
            print(
                "processing %d / %d; accumulated %d frames; took %.fs"
    ...
    print("")
    print("Took %.fs to dump %d code frames for %d utts" % (t1 - t0, tot_nframes, n_utts))
...
def run_analysis(utt2codes, utt2words, stop_words, output_dir):
    code_to_wordprec = load_or_run_and_dump(
        "%s/code_to_wordprec.pkl" % output_dir, comp_code_to_wordprec, [utt2codes, utt2words, 0.04, stop_words], {}
    )

    word_to_coderecall = load_or_run_and_dump(
        "%s/word_to_coderecall.pkl" % output_dir, comp_word_to_coderecall, [utt2codes, utt2words, []], {}
    )

    high_prec_words = load_or_run_and_dump(
        "%s/high_prec_words.pkl" % output_dir, get_high_prec_words, [code_to_wordprec], {"threshold": 0.35}
    )
...
if __name__ == "__main__":
    ...
    utt2codes, utt2words = load_or_run_and_dump(
        ...
        prepare_data,
        [audio_model, layer, dataset, json_data, max_n_utts],
        {},
    )
    code_norms = load_or_run_and_dump("%s/code_norms.pkl" % output_dir, comp_code_norms, [audio_model, layer], {})

    c2wp, w2cr, hp_ws = run_analysis(utt2codes, utt2words, STOP_WORDS, output_dir)

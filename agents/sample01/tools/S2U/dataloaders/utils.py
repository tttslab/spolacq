...
def compute_spectrogram(y, native_sample_rate, audio_conf={}):
    ...
    y = librosa.resample(y, orig_sr=native_sample_rate, target_sr=sample_rate)
    ...
    # compute spectrogram
    stft = librosa.stft(y, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=WINDOWS[window_type])
    spec = np.abs(stft) ** 2

...
def get_dataloader(input_dir, input_ext, batch_size):
    ...
    loader = DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True)
    return loader
...
def extract_and_write_features(audio_model, layer, loader, output_dir, output_name_level):
    ...
    with torch.no_grad():
        for idx, ((audio_input, n_frames, paths), _) in enumerate(loader):
            ...
            (_, q_out, preq_out, onehot) = audio_model.get_vq_outputs(audio_input, vq_layer, True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_ext", type=str, default="wav", help="Extension for waveform files to search in input directory"
    )
    parser.add_argument("--batch_size", type=int, default=100, help="Batch size")
    parser.add_argument(
        "--output_name_level", type=int, default=1, help="Concat how many levels of original path for output name."
    )
    ...
    extract_and_write_features(audio_model, args.layer, loader, args.output_dir, args.output_name_level)

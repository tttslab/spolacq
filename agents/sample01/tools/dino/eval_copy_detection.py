    ...
    def eval_result(self, ids, distances):
        ...
        for i in range(self.nblocks):
            ...
            if block_name != "strong":
                ...
                positives_per_query = [
                    [j for j, bname in enumerate(originals) if bname[:4] == qname[:4]] for qname in strongs
                ]
...
def extract_features(image_list, model, args):
    ...
    for samples, index in utils.MetricLogger(delimiter="  ").log_every(data_loader, 10):
        ...
        feats = nn.functional.avg_pool2d(feats.pow(4), (h, w)).pow(1.0 / 4).reshape(b, -1)
        ...
        feats_all = torch.empty(
            dist.get_world_size(), feats.size(0), feats.size(1), dtype=feats.dtype, device=feats.device
        )

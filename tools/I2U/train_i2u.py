...
def train(device, loader, model, reconstruction_loss, optimizer):
    ...
    for imgs, units, seq_lens, padding_masks in loader:
        ...
        logits, _, _, _ = pack_padded_sequence(logits, (seq_lens-1).cpu().tolist(),
                                               batch_first=True, enforce_sorted=False)
        targets, _, _, _ = pack_padded_sequence(units[:, 1:], (seq_lens-1).cpu().tolist(),
                                                batch_first=True, enforce_sorted=False)
...
def main(config):
    ...
    for epoch in range(1, 1 + config["I2U"]["epoch"]):
        ...
        print("epoch", epoch, "train_accuracy", train_accuracy, "train_loss", train_loss,
              "val_accuracy", val_accuracy, "val_loss", val_loss, flush=True)

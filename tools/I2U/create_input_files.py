...
def add_indications(name, idx):
    ...
    ll = [
        name,
        f"{preposition} {name}",
        f"{preposition2} {color} {name}",
        f"i want {preposition} {name}",
    ]
...
def make_mp3_wav(food_name, text, i):
    ...
    if not os.path.isfile(f"../../data/I2U/audio/{food_name}/{food_name}_{i}.wav"):
        ...
        ffmpeg.run(
            ffmpeg.output(
                ffmpeg.input("../../data/I2U/audio/tmp.mp3"),
...
if __name__ == "__main__":
    ...
    train_image_paths = sorted(
        glob.glob("../../data/dataset/*/train_number[123]/*.jpg")
    )
    ...
    data_folder = os.path.join(
        os.path.dirname(__file__), "../..", config["I2U"]["data_folder"]
    )

...
class WaveGrad(nn.Module):
  def __init__(self, params, cond_net):
    super().__init__()
    ...
    self.upsample = nn.ModuleList([
        ...
        #UBlock(128, 128, 2, [1, 2, 4, 8]), #for 16khz
    ])
    #self.first_conv = Conv1d(128, 768, 3, padding=1)
    self.first_conv = Conv1d(50, 768, 1)
    self.last_conv = Conv1d(128, 1, 3, padding=1)

  def forward(self, audio, condition, noise_scale):
    x = audio.unsqueeze(1)
    downsampled = []
    ...
    condition = self.cond_net(condition).unsqueeze(2)
    x = self.first_conv(condition)
